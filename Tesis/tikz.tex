...
\draw (4,0) ellipse [x radius=5mm, y radius=7mm];
...
\begin{center}
\begin{tikzpicture}
\node at (0,0) {\(\circ\)}; % nodo de referencia
\node at (1,1) {\(\circ\)}; % nodo de referencia
\node at (2,1) {\(\circ\)}; % nodo de referencia
\node at (2,0) {\(\circ\)}; % nodo de referencia
\draw (0,0) .. controls (1,1) and (2,1) .. (2,0);
\end{tikzpicture}
\end{center}

Ahora vamos a colorear figuras
\begin{center}
\begin{tikzpicture}
...
Estas formas básicas de usar Tikz pueden tener muchas posibilidades si se
combinan entre ellas.

Ahora a graficar. En las funciones trigonométricas se debe especificar como se hacen los cálculos: radianes, grados, etc. Para obtener el seno como queremos se escribe \verb|sin((\y)r)|.
\begin{center}
\begin{tikzpicture}
...
Otra opción para graficar es usar el paquete \texttt{pgfplots} que usará el motor de \texttt{pgf} para hacer gráficas. Es importante notar que use \verb|\pgfplotsset{width=6cm,compat=1.17}| en el preámbulo. La parte de \texttt{width} es para que todas las gráficas tengan el mismo tamaño y así tener uniformidad en el texto. La parte de \texttt{compat} es la versión del motor que se usará, diferentes versiones podrían generar diferentes salidas con el mismo código.
\begin{center}
\begin{tikzpicture}
\begin{axis}[
    xmin=-0.1,xmax=1,
...
\begin{center}
\begin{tikzcd}
T\ar[bend left=20]{rrd}{h_{1}}\ar[bend right=20]{rdd}[swap]{h_{2}}\ar[dashed]{rd}[description]{h}\\[-4mm]
&[-3mm] P\ar{r}{p_{1}}\ar{d}{p_{2}} & A\ar{d}{f}\\
& B\ar{r}[swap]{g} & C
\end{tikzcd}
\end{center}
